...
/** Local shim (not @react-types/shared). */
...
export interface CalendarPropsBase {
    ...
    /**
     * When `true`, next/previous page moves the focused date with the same calendar step
     * (e.g. April 16 → May 16). When `false`, page changes do not update focused date
     * (avoids stretching range preview on native when anchor + focused differ).
     *
     * @default `true` on web, `false` on iOS and Android
     */
    preserveFocusedDayOnPage?: boolean;
}
