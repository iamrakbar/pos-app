export declare const segmentClassNames: import("../../helpers/internal/types").CombinedStyles<{
    root: import("tailwind-variants").TVReturnType<{
        isDisabled: {
            true: string;
            false: string;
        };
    }, undefined, "", {
        isDisabled: {
            true: string;
            false: string;
        };
    }, undefined, import("tailwind-variants").TVReturnType<{
        isDisabled: {
            true: string;
            false: string;
        };
    }, undefined, "", unknown, unknown, undefined>>;
    group: import("tailwind-variants").TVReturnType<{
        size: {
            sm: string;
            md: string;
            lg: string;
        };
    }, undefined, "", {
        size: {
            sm: string;
            md: string;
            lg: string;
        };
    }, undefined, import("tailwind-variants").TVReturnType<{
        size: {
            sm: string;
            md: string;
            lg: string;
        };
    }, undefined, "", unknown, unknown, undefined>>;
    scrollView: import("tailwind-variants").TVReturnType<{
        size: {
            sm: string;
            md: string;
            lg: string;
        };
    }, undefined, "", {
        size: {
            sm: string;
            md: string;
            lg: string;
        };
    }, undefined, import("tailwind-variants").TVReturnType<{
        size: {
            sm: string;
            md: string;
            lg: string;
        };
    }, undefined, "", unknown, unknown, undefined>>;
    scrollViewContentContainer: import("tailwind-variants").TVReturnType<{
        size: {
            sm: string;
            md: string;
            lg: string;
        };
    }, undefined, "", {
        size: {
            sm: string;
            md: string;
            lg: string;
        };
    }, undefined, import("tailwind-variants").TVReturnType<{
        size: {
            sm: string;
            md: string;
            lg: string;
        };
    }, undefined, "", unknown, unknown, undefined>>;
    item: import("tailwind-variants").TVReturnType<{
        size: {
            sm: string;
            md: string;
            lg: string;
        };
        isDisabled: {
            true: string;
            false: string;
        };
    }, undefined, "segment__item", {
        size: {
            sm: string;
            md: string;
            lg: string;
        };
        isDisabled: {
            true: string;
            false: string;
        };
    }, undefined, import("tailwind-variants").TVReturnType<{
        size: {
            sm: string;
            md: string;
            lg: string;
        };
        isDisabled: {
            true: string;
            false: string;
        };
    }, undefined, "segment__item", unknown, unknown, undefined>>;
    label: import("tailwind-variants").TVReturnType<{
        size: {
            sm: string;
            md: string;
            lg: string;
        };
    }, undefined, undefined, {
        size: {
            sm: string;
            md: string;
            lg: string;
        };
    }, undefined, import("tailwind-variants").TVReturnType<{
        size: {
            sm: string;
            md: string;
            lg: string;
        };
    }, undefined, undefined, unknown, unknown, undefined>>;
    indicator: import("tailwind-variants").TVReturnType<{
        size: {
            sm: string;
            md: string;
            lg: string;
        };
        isScrollView: {
            true: string;
            false: string;
        };
    }, undefined, "", {
        size: {
            sm: string;
            md: string;
            lg: string;
        };
        isScrollView: {
            true: string;
            false: string;
        };
    }, undefined, import("tailwind-variants").TVReturnType<{
        size: {
            sm: string;
            md: string;
            lg: string;
        };
        isScrollView: {
            true: string;
            false: string;
        };
    }, undefined, "", unknown, unknown, undefined>>;
}>;
//# sourceMappingURL=segment.styles.d.ts.map