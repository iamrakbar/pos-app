...
type CheckoutResultData = NonNullable<ReturnType<typeof usePOSStore.getState>["checkoutResult"]>;

function PaymentSuccessSummary({
  paymentSession,
  checkoutResult,
  paidAt,
  totalQty,
  ...
  checkoutResult: CheckoutResultData | null;
  paidAt: Date;
  totalQty: number;
}): JSX.Element {
...
function PaymentSuccessActions({
  ...
  isCompact: boolean;
  receiptSettings: ReturnType<typeof useReceiptStore.getState>["settings"];
  isPrinting: boolean;
  themeColorForeground: string;
  updateReceiptSettings: ReturnType<typeof useReceiptStore.getState>["updateSettings"];
  onPrint: () => void;
  onNewOrder: () => void;
}): JSX.Element {
  const { t } = useTranslation();
