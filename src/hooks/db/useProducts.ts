...
import { extractCategoryId, extractOriginalPrice, type RawPosProduct } from '@/api/mappers/product';
import { useAuth } from '@/stores/useAuth';
import type { POSProduct } from '@/types/pos';

const PRODUCTS_STALE_TIME_MS = 5 * 60 * 1000;

export function mapProduct(raw: RawPosProduct): POSProduct {
    return {
        id: raw.id,
        name: raw.name,
        price: raw.price,
        ...
        category_id: extractCategoryId(raw.category),
...
export function useProductsRaw(search?: string, categoryId?: string | null) {
    ...
    return useQuery({
        ...
        queryFn: async () => {
            ...
            return res.data as RawPosProduct[];
        },
        enabled: !!merchantId,
