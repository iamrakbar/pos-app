...
export function extractPaymentQrUrl(checkout: {
...
export function extractPaymentLink(checkout: {
  payment_details?: unknown;
  payment: unknown;
}): string | null {
  for (const value of [checkout.payment_details, checkout.payment]) {
    const rec = asReadableRecord(value);
    if (!rec) continue;
    for (const key of ["payment_url", "payment_link", "checkout_url", "link", "url"]) {
...
export function extractPaymentInstruction(
  ...
  const candidates = [rec[locale], rec.en, rec.id, rec.instruction, rec.text, rec.description];
  return (
    candidates
      .find(
